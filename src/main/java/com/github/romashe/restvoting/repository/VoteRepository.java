...
public interface VoteRepository extends BaseRepository<Vote> {
    ...
    @Query("SELECT v.restaurantId as restaurantId, count(v.id) as voteCount FROM Vote v" +
            " WHERE v.voteDate = CURRENT_DATE GROUP BY v.restaurantId")
    List<RestaurantVoteCnt> findAllTodayVotesAsList();

    default Map<Integer, Long> findAllTodayVotes() {
        return findAllTodayVotesAsList().stream()
                .collect(Collectors.toMap(RestaurantVoteCnt::getRestaurantId, RestaurantVoteCnt::getVoteCount));
    }
